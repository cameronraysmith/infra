...
The management cluster SHALL be exposed as `apps.<system>.k8s.k8s-mgmt-k3d` (handler B: a k3d cluster on stibnite's Colima Docker; `modules/kubernetes/capi/management.nix`), and the contract every handler satisfies SHALL be: a kubeconfig on the operator host, the Cluster API core, CAPH, and cluster-api-k3s providers `Ready`, the rendered `capi-<c>` tree and the T0 `Secret`s applied unchanged, and `clusterctl move` to the workload cluster succeeding.
...
- **WHEN** any object in the rendered `capi-<c>` tree or any T0 `Secret` references k3d, Colima, Docker, or a QEMU artifact and `checks.k8s-capi-render-<c>` is rebuilt
...
Handler B SHALL require nothing beyond the repository checkout, the decryptable Clan vars under `secrets/clusters/<c>/`, one Hetzner API token supplied as a `Secret`, and stibnite with Colima; it SHALL NOT require a seed host, a standing Hetzner machine, a fleet host, ZeroTier, or any Clan overlay.
...
`apps.<system>.k8s.clusterctl-move` SHALL run `clusterctl move --to-kubeconfig <c>` after both nodes are `Ready` and Flux's root `Kustomization` is `Ready=True`, SHALL assert that the `Cluster`, its control plane, its `MachineDeployment`, and every `Secret` labelled `cluster.x-k8s.io/cluster-name=<c>` are present in the workload cluster afterwards, and the handler-B recipe SHALL then delete the k3d cluster so that no management cluster remains on stibnite.
