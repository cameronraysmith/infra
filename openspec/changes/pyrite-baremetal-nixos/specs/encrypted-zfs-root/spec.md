...
- **AND** that entry carries no `fido2-device=auto` option as installed, because disko emits `crypttabExtraOpts` under `lib.mkIf config.enrollFido2` (`lib/types/luks.nix:348`) and `enrollFido2` is false, so `determine_token_type` returns `_TOKEN_TYPE_INVALID` and stage 1 requests the passphrase directly with no token attempt and no preceding wait
- **AND** the option is added by hand after both tokens are enrolled, because an enrolled header is not by itself sufficient: `determine_token_type` (systemd 260.2 `src/cryptsetup/cryptsetup.c:2551-2560`) selects `TOKEN_FIDO2` at `:2555` only from `arg_fido2_device` or `arg_fido2_device_auto`, which crypttab's `fido2-device=` (`:394-405`) and `fido2-cid=` (`:424-426`) set — upstream's `luksroot.nix:614` hedge that systemd "usually" detects the configuration at runtime does not cover the FIDO2 case, and its own "if necessary, configure the corresponding crypttab(5) options" clause is the whole of the mechanism here
