...
- **AND** that entry carries no `fido2-device=auto` option as installed, because disko emits `crypttabExtraOpts` under `lib.mkIf config.enrollFido2` (`lib/types/luks.nix:348`) and `enrollFido2` is false, so `determine_token_type` returns `_TOKEN_TYPE_INVALID` and stage 1 requests the passphrase directly with no token prompt of any kind and no preceding wait
- **AND** the option is added by hand after both tokens are enrolled, because an enrolled header is not by itself sufficient: `determine_token_type` (systemd 260.2 `src/cryptsetup/cryptsetup.c:2551-2560`) selects `TOKEN_FIDO2` at `:2555` only from `arg_fido2_device` or `arg_fido2_device_auto`, which crypttab's `fido2-device=` (`:394-405`) and `fido2-cid=` (`:424-426`) set — upstream's `luksroot.nix:614` hedge that systemd "usually" detects the configuration at runtime names the libcryptsetup token-plugin path at `:2691`, which is attempted on this machine and loads no token module, so upstream's own "if necessary, configure the corresponding crypttab(5) options" clause is the whole of the mechanism here
- **AND** no stage-1 option is added for this: `boot.initrd.systemd.fido2.enable` defaults to `config.boot.initrd.systemd.package.withFido2` at `nixos/modules/system/boot/systemd/fido2.nix:12-15`, and its config block at `:19-30` already places `60-fido-id.rules`, `fido_id`, `libcryptsetup-token-systemd-fido2.so`, and `libfido2.so.1` inside the initrd — all four confirmed present by extracting the deployed initrd, whose `/init` symlinks to systemd; presence in the initrd and presence on libcryptsetup's search path are separate facts here, because the plugin is installed under the systemd prefix (`…-systemd-260.2/lib/cryptsetup/`) while libcryptsetup's compiled token directory (`…-cryptsetup-2.8.6/lib/cryptsetup`) is empty — measured on 2026-07-30, not verified at runtime, and reasoned to its consequence at `cryptsetup.c:2691` in `design.md`
...
- **AND** at the end state both tokens carry a FIDO2 client PIN, so `systemd-cryptenroll`'s default `--fido2-with-client-pin=yes` applies and the unlock is a typed PIN followed by a physical touch; before the crypttab option lands, the unlock is the passphrase typed on the same keyboard, so the SPI keyboard requirement recorded in `apple-laptop-hardware-support` is load-bearing in both states rather than only the end state
