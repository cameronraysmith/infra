...
- To: `modules/machines/nixos/pyrite/disko.nix` declares an `EF00`-typed ESP and a sibling partition holding a LUKS2 container, on a namespace-explicit `/dev/disk/by-id` path, with the ZFS `zroot` pool created inside that container at `options.ashift = "12"`. The install creates the container with a human-typeable passphrase from a clan `neededFor = "partitioning"` vars generator as its only keyslot, and both FIDO2 tokens are enrolled afterward on the booted machine with `systemd-cryptenroll` (D30), so the end state is a boot that prompts on the internal keyboard for a token PIN and falls back to the passphrase when no token is present. No dataset carries ZFS native encryption.
- Reason: ZFS-on-root matches the fleet's five other NixOS machines, and the container sits below the pool rather than replacing it, so that parity is kept; encryption moves to LUKS because it is the only layer offering keyslots, and keyslots are what let a hardware token and a recovery passphrase coexist on a machine with no initrd SSH and no possibility of a remote reboot — ZFS permits one key per encryption root and `zfs change-key` replaces rather than adds. the install is non-interactive and re-runnable because `enrollFido2` is false: disko runs `systemd-cryptenroll` as a bare body command under `set -efux` after `luksFormat` has already replaced the container, with no retry and no fallback, so a single declined touch strands a machine that has no operating system at that moment — which is what happened on 2026-07-30 and what D30 defers; `ashift = "12"` is mandatory on this disk's 4096-byte physical sectors and more so now that the vdev is a mapper device reporting its own geometry; `type = "EF00"` is mandatory because disko defaults to `8300` and Apple's EFI discovers the ESP by type; the namespace-explicit path is mandatory because the controller exposes an 8 KiB second namespace sharing a by-id prefix.
