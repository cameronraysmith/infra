...
> **Superseded.** This plan is retained as a record of the change's earlier shape and is not the operative artifact.
> The operative artifacts are `tasks.md`, for what remains to be done and in what order, and `docs/notes/development/hardware/pyrite-install-runbook.md`, for how to perform it.
> `design.md` remains authoritative for the decisions.
> The body below is not corrected, because correcting it would produce a second artifact competing with `tasks.md` rather than a better plan.
> It last moved on 2026-07-17 and predates D1, so it is stale in five ways a reader would otherwise have to discover: it specifies ZFS native encryption with `aes-256-gcm` and a `postCreateHook` flipping `keylocation` to `prompt`, where the change now specifies a LUKS2 container with FIDO2 unlock and no dataset-level encryption at all; its Task 7 Step 2 prescribes `--phases disko`, which `tasks.md` 7.3 forbids outright because zeroing the kexec phase rewrites the ssh connection to `root@` and hangs forever on key upload after the wipe; its "Approach and sequencing" paragraph on Task 7 states in the present tense that step 7.12 re-runs the install including the wipe, where D29 has since dropped that second destructive install entirely and the change now performs exactly one; its "Out of scope" paragraph lists the `d3cold` suspend workaround, where D21 and D22 have since brought suspend and resume into scope as design.md's Goal 5 and tasks.md's Phase 9; and its step numbering does not correspond to the task numbering that has since been used everywhere else.
> The checkboxes below are not maintained: `tasks.md` carries the ledger of what has been done, and a box here is unchecked whether or not the work it names was performed.
> Reconciling them against `tasks.md` is not a matter of ticking boxes: the two checklists do not enumerate the same steps, `tasks.md` carries a ninth phase this plan has no counterpart for, and this plan's Task 5 Step 1 prescribes standing up a dedicated fleet SSID that `tasks.md` 5.1 records as work the operator decided not to do at all.

> **For agentic workers:** Use superpowers:subagent-driven-development
> to implement `tasks.md` task-by-task.
...
- [ ] **Step 4:** Disable `boot.initrd.network.ssh` only — never `mkForce` `boot.initrd.kernelModules`. State `hardware.enableRedistributableFirmware = true` and `hardware.cpu.intel.updateMicrocode = true` explicitly rather than inheriting facter's bare-metal `mkDefault`s, keep `forceImportRoot = true` with its comment, set no `hostId`, and enable no plymouth. Set `services.mbpfan.aggressive = false`; `services.mbpfan.enable` stays at the profile's `mkDefault true` and runs, while `services.tlp.enable` evaluates `false` because the profile defaults it to `lib.mkDefault (!config.services.power-profiles-daemon.enable)` and GNOME (D19) enables `services.power-profiles-daemon.enable`, the machine's active power governor. The module restates neither `enable`. See D16. Enable a stock GNOME desktop with `services.displayManager.gdm.enable = true` and `services.desktopManager.gnome.enable = true` — system-level, with no home-manager desktop configuration, and no plymouth (D11 stands; GDM is a stage-2 display manager that does not touch the initrd passphrase path). See D19.
