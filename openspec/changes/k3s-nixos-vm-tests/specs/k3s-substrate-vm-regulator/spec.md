...
- **THEN** within the test timeout `kubectl get node cryolite-agent` on the server succeeds and shows the agent's role label
...
- **WHEN** the `cryolite` Clan vars generator for the k3s token exists with `share = true`
