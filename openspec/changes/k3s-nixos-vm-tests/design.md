...
Revision 2 of this design (2026-09-04, scope review; numbered as the ADRs number it — the initial #2955 draft, then the D1–D31 fold as revision 1, then this) narrows every property to one new sibling cluster `kubernetes/clusters/<c>`; the frozen prototypes `local` and `local-k3d` keep ArgoCD, nixidy, sops-secrets-operator, kube-proxy, and the k3d workflow, and no artifact in this change edits them.
...
- Existing files grow only additively through default-off options or lose only dead code (ADR-007 D7.15): `modules/kubernetes.nix` (`evalCluster` gains `<c>`), `modules/nixos/k3s-server/default.nix` (`snapshotter` option; inert containerd block deleted), `modules/devshells/kubernetes.nix` (flux, clusterctl, hcloud, cosign, crane).
...
This tree is the single record of where the `<c>` architecture lives (ADR-007 D7.17); the sibling change references it rather than restating it.
`[keep]` paths are untouched by either change; `[add]` paths are new and dendritically additive, so deleting them makes `<c>` disappear with no other edit; `[+opt]` paths are existing files that gain a default-off option or lose dead code.
...
```text
modules/
  kubernetes.nix                        [+opt] evalCluster gains <c>
  nixidy.nix                            [keep]
  kubernetes/                           [add]
    README.md
    artifacts.nix                       k8s-manifests-<c>, k8s-oci-<c> (OCI image-layout drv; digest = build output)
    oci-lib.nix                         mkOciLayout, mkNixSnapshotterImage, readDigest
    purity.nix                          checks.k8s-purity-<c> (S0)
    provenance.nix                      checks.k8s-provenance-<c> (S0)
    identity.nix                        checks.k8s-node-identity-free-<c> (S0/S3)
  ...
  nixos/
    ...
    k3s-flux.nix                        [add]
  machines/nixos/<c>-server.nix, <c>-agent.nix   [add] image-only, role fixed at build
  checks/
...
kubernetes/
  ...
  clusters/<c>/{default.nix, topology.nix}  [add]
  nixidy/local-k3d/                     [keep]
  tests/local-k3d/                      [keep]
  tests/<c>/                            [add]
secrets/clusters/<c>/                   [add]
```
...
- Dendritic additivity: every `[add]` path is reachable only from `evalCluster`'s `<c>` entry, so removing the `[add]` set removes the cluster.
- One evaluation, one hash chain: `clusters/<c>` → `k8s-manifests-<c>` → `k8s-oci-<c>` digest → `k3s-flux.nix` pins the digest → `<c>-server` closure → snapshot label → `capi-<c>` CRs.
...
- Regulate the `<c>` core — Cilium with kube-proxy replacement, Flux from a digest-pinned OCI artifact, easykubenix rendering, Cilium LB-IPAM, cert-manager and step-ca — in a hermetic two-guest VM against the production envelope.
- Regulate the rendered `<c>` tree and its artifacts without KVM (purity, preload coverage, provenance, node-identity freedom, pinned sources) so the buildbot worker regulates every push.
...
Revision-3 status is given in brackets: `[kept]`, `[narrowed]` (now `<c>`-only), `[moved]` (to the sibling change), or `[superseded]` with the replacing decision.

### D1: Five tiers, assigned per assertion [narrowed]

Every assertion of the `<c>` core is placed at T1 (pure eval/build), T2 (VM substrate), T3 (VM platform), E (effect, never a check), or K (k3d, only as the sibling change's management handler B) per the ADR-007 Q1 table.
The k3d-only properties — Docker volume mount consumption and behavior under kube-proxy plus ServiceLB — are properties of `local-k3d`, stay regulated by its own workflow, and are not properties of `<c>`.
...
Alternative considered: a permanent k3d residue for the `<c>` suite (O-3); rejected because every blocker in ADR-007 Q4 has a hermetic substitute.
...
### D3: T1 leaves regulate the evaluated module and the rendered `<c>` tree [narrowed]
...
`k8s-purity-<c>` (`modules/kubernetes/purity.nix`), `k8s-provenance-<c>` (`provenance.nix`), `k8s-node-identity-free-<c>` (`identity.nix`), `flux-sources-pinned`, and `flux-install-rendered` are `runCommand` leaves over the easykubenix-rendered tree of `<c>` (ADR-008 D8.11, D8.12, D8.4, D8.2; ADR-010 D10.6).
Revision 1's `k8s-manifest-purity` and `k8s-images-preloaded` are the two assertion groups of `k8s-purity-<c>`; `k8s-closure-provenance` is `k8s-provenance-<c>`; the rename follows the module tree.
...
Revision 2 has no such leaf: `vm-k3s-platform` boots the `<c>` server and agent (D6), so the join assertions — both nodes registered, the agent's `ExecStart` carries `--server=` and no `--cluster-cidr`/`--service-cidr`, the agent reaches `server:6443` and `server:10250` through the production firewall — live there, with Cilium present, and pod-to-pod connectivity across nodes is asserted alongside.
...
`vm-k3s-platform` boots `<c>-server` and `<c>-agent` (both importing `base` and `k3s-server`; the agent with `role = "agent"` and `serverAddr` pointing at the server over one test-driver VLAN), preloads every image the rendered tree references through `services.k3s.images` (the set is derived from the tree by string context, ADR-008 D8.11), applies Cilium with `kubeProxyReplacement=true` and the Flux install through `services.k3s.manifests`, runs a registry in the server guest seeded from the sandbox-built OCI layout (ADR-008 D8.6), renders the root `OCIRepository` with that layout's digest and a `spec.verify` public key from a test-only cosign pair, and runs `chainsaw test` on `kubernetes/tests/<c>` from a store path with `KUBECONFIG=/etc/rancher/k3s/k3s.yaml`.
The `<c>` suite has two phases: foundation (Cilium `Ready`, Flux root `Kustomization` `Ready=True` with `status.lastAppliedRevision` equal to the pinned digest) and infrastructure (cert-manager, step-ca, `ClusterIssuer`, `Gateway`, `Certificate`s, `HTTPRoute`).
It is authored for `<c>`, not copied from `kubernetes/tests/local-k3d`, and carries no ArgoCD or sops-secrets-operator step.
...
The Gateway LoadBalancer address is supplied by a `CiliumLoadBalancerIPPool` declared by the `<c>` cluster module (ADR-007 D7.9); ServiceLB stays disabled and kube-proxy is replaced by Cilium.
...
Production uses per-cluster Clan vars generators for both, as T0 identity under `secrets/clusters/<c>/` (ADR-008 D8.9, D8.14; ADR-010 D10.1); the uncovered production properties are listed in the `k3s-platform-vm-regulator` spec.

### D9: easykubenix for `<c>`; a VM target of the `<c>` cluster module, not a nixidy environment [narrowed]

The rendered tree is produced by the easykubenix cluster module `kubernetes/clusters/<c>/` with a `target` that selects VM fixtures (hostnames, CoreDNS answer, registry URL, key material) from production values.
No nixidy environment is created for `<c>`; `modules/nixidy.nix` and `kubernetes/nixidy/local-k3d/` keep serving `local-k3d` and are not edited.
...
S4 spends money and starts only on explicit words; it is the sibling change's D10 and nothing in this change's stages touches a cloud.

### D14: `<c>` is a sibling, and the prototypes are frozen [new, rev 2; ADR-007 D7.15]

Every regulator, module, artifact, and secret this change plans is keyed by `<c>`; `evalCluster` gains one entry; nothing under `kubernetes/clusters/local*`, `kubernetes/nixidy/`, `kubernetes/tests/local-k3d/`, or `modules/apps/cluster/` is read differently or edited.
ArgoCD (for `local-k3d`) and Flux (for `<c>`) coexist; this is a per-cluster reconciler choice, not a migration.
...
- R2 platform closure size: bounded at 1.5–2.5 GiB compressed, not measured; two guests double the memory footprint. Mitigation: `k8s-provenance-<c>` reports it in S0; if S2 wall time exceeds 15 minutes, apply O-2.
...
- R7 the `<c>` Chainsaw suite is new and its first green run is also its first run. Mitigation: each assertion has a recorded mutation (tasks S2.7); the `local-k3d` suite is left as an independent oracle of the shared components' behavior under the old envelope.
...
- R10 the frozen prototypes drift from `<c>` as `<c>` evolves (two reconcilers, two secret mechanisms). Accepted by the scope review; the feedback change owns convergence or retirement.

## Migration Plan

1. S0 lands the T1 leaves listed in D3 against an initial `kubernetes/clusters/<c>/` with `platform = hetzner` only, `modules/kubernetes/{artifacts,oci-lib,purity,provenance,identity}.nix`, `modules/kubernetes/flux/`, and the `apps.k8s.oci-push` and `apps.k8s.cosign-sign` effects (not run); the k3d workflow is untouched.
...
4. S2 lands `kubernetes/tests/<c>/`, `modules/nixos/k3s-flux.nix`, `machines/nixos/<c>-{server,agent}.nix`, the age and cosign fixtures, the LB-IPAM pool, and `vm-k3s-platform`; unchanged workflow.
...
- A8 delta-spec numbering: the `world-assumptions` additions are numbered A13–A16 here and A17–A19 in the sibling change, assuming the two unmerged changes that add A9–A12 land first. Recommended: renumber at sync if not.
- A10 the name of `<c>` (ADR-007 OQ7.1). Recommended: a mineral name consistent with the fleet (stibnite, magnetite, cinnabar); `<c>` stays a placeholder in every artifact until named, and the rename is search-and-replace.
- A11 whether the `<c>` Chainsaw suite reuses the `local-k3d` step YAML for cert-manager, step-ca, and Gateway by copy or references shared assert files. Recommended: copy into `kubernetes/tests/<c>` and edit there; sharing files would couple the frozen prototype to `<c>`, which D14 forbids.
- A12 whether the agent guest in `vm-k3s-platform` should also host a registry mirror so that image pulls on the agent do not depend on the server guest. Recommended: no; images arrive on both guests through `services.k3s.images`, and the registry serves only the Flux artifact to the server's source-controller.
