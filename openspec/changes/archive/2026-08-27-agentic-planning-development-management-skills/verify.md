...
| Compose-by-delegation (D4) | router never re-implements orient/plan/review/checkpoint; Manual pass-through to /session-orient; session-advisor referenced not duplicated | agentic-workflow-routing "Compose by delegation, never re-implement" | session-advisor was deleted after this design.md was written; the spec has been amended to drop the session-advisor clause and Manual is now a direct pass-through to /session-orient, so design.md's mention is superseded historical record, not a live gap. This row was re-checked against `specs/agentic-workflow-routing/spec.md` after that amendment landed (the amendment removed the session-advisor requirement and scenario before this re-check), and confirms the live requirement text carries no session-advisor reference and no gap remains. |
