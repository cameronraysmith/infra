...
Proposed (2026-09-04, revision 2 of the k3s VM-test design).
Design only; no flake input, production module, Clan inventory, workflow, or cloud change lands with this record.
Implemented by S0 of `openspec/changes/k3s-nixos-vm-tests/` (the identity-free regulator) and by S3, S4, and S4b of `openspec/changes/capi-hetzner-cluster/` (delivery, deployment, admin overlay); S4 spends money and needs explicit written approval.
Every decision governs the sibling cluster `kubernetes/clusters/<c>` (ADR-007 D7.15).

## Context

ADR-009 makes the nodes of `<c>` Cluster API objects that CAPH creates from a snapshot and deletes on every flake bump.
...
| L2 management cluster | k3d on stibnite through Colima (handler B, ADR-009 D9.17): `clusterctl init` with CAPI core, CAPH, and cluster-api-k3s through a `clusterctl.yaml` override (ADR-009 F9.2); T0 Secrets and the `capi-<c>` CRs applied | bootstrap only; deleted after pivot |
| L3 workload cluster | `<c>`: two Hetzner nodes created by CAPH; Flux reconciles from the digest in the closure (ADR-008 D8.4); after `clusterctl move` from L2 it hosts its own CAPI providers | permanent |
...
Every T0 item a node needs is a `KThreesConfig.spec.files[]` entry with `contentFrom.secret{name,key}` (F10.2) naming a Secret in the management cluster's namespace for `<c>`, created from the sops-decrypted Clan vars by the L2 apply step:
...
There is one snapshot per role (`<c>-server`, `<c>-agent`; ADR-009 D9.16) because `services.k3s.role` is fixed at evaluation (ADR-009 F9.5), and the two differ in nothing that is identity.
...
`checks.<system>.k8s-node-identity-free-<c>` (`modules/kubernetes/identity.nix`, ADR-007 D7.17) evaluates the rendered `capi-<c>` CRs, the rendered Flux artifact, the Clan inventory, and the sops file index under `secrets/clusters/<c>/`, and fails if any of them names a node or a node key: a `Machine` name pattern, a node hostname, an SSH host key or its fingerprint, a Cilium WireGuard public key, or a `CiliumNode` name.
It is KVM-free (T1) and runs from S0 with the CRs stubbed to what `kubernetes/clusters/<c>/` renders at that stage; from S3 it consumes the real render.
...
|---|---|---|---|
| R10.1 | no CAPI CR, Flux manifest, Clan inventory entry, or Secret index names a node or node key | `k8s-node-identity-free-<c>` | T1 |
| R10.2 | the per-role snapshot closure contains no T0 private material | a T1 leaf that greps the closure for the sops file paths and key fingerprints of `secrets/clusters/<c>/` | T1 |
...
- OQ10.1 The etcd-S3 bucket is the one external dependency of D10.7; which provider hosts it (Hetzner Object Storage, an existing bucket elsewhere) is unstated. Recommendation: Hetzner Object Storage in the same project so that L0's Hetzner token covers it, decided in the S4 PR.
- OQ10.2 Whether the SSH CA signs host keys at boot on the node (D10.4) or the operator signs a per-`Machine` certificate offline is a choice between one transient private-key exposure per node and an operator step per node roll. Recommendation: on-node signing as written, with R10.e discharged in S3.
...
- ADR-009: CAPI, the air-gapped shim (D9.2), `files[]` delivery (D9.7), the image path (D9.16), the overlay whose gateway key is T0 (D9.15), and handler B (D9.17).
