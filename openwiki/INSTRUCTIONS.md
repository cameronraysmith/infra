...
The root `AGENTS.md` is generated by apm from the `*.instructions.md` fragments of the `agent-context-*` packages under `modules/home/ai/plugins/` and is not committed, so it is absent from fresh checkouts; those fragments are the authoritative context wherever they and the wiki disagree.
