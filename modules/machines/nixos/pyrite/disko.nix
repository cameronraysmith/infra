            ...
            partitions = {
              ...
              zfs = {
                ...
                content = {
                  ...
                  # Enrollment deferred to first boot, not declined. disko runs it as a bare
                  # body command under the `set -efux` of lib/default.nix:1012, after
                  # luksFormat has already replaced the container (lib/types/luks.nix:244,
                  # then :275-302), with no retry and no fallback: the 2026-07-30 attempt
                  # failed the touch with FIDO_ERR_OPERATION_DENIED and left a formatted
                  # disk and no operating system. false also clears autogeneratedPassword
                  # (:12), resolving formatKeyFile to passwordFile (:32-33), so luksFormat
                  # puts the clan-vars passphrase itself in slot 0, the additionalKeyFiles
                  # `open --test-passphrase` guard at :258 succeeds and adds no second slot,
                  # and the install runs with no prompt of any kind. The token is enrolled
                  # afterward with systemd-cryptenroll on the booted machine, where a failed
                  # touch costs a retry. Token unlock at boot additionally needs the
                  # crypttabExtraOpts that :348 stops emitting under this false, which must
                  # not ship before the header carries a credential; see default.nix.
                  enrollFido2 = false;
