            ...
            partitions = {
              ...
              zfs = {
                ...
                content = {
                  ...
                  # Deferred to first boot, not declined: disko enrolls as a bare body
                  # command under the `set -efux` of lib/default.nix:1012, sequenced after
                  # luksFormat has already replaced the container (lib/types/luks.nix:244,
                  # :275-302), so a failed enrollment aborts post-wipe onto a machine with
                  # no operating system. false also clears autogeneratedPassword (:12), so
                  # passwordFile becomes the format key (:32-33): slot 0 holds the clan-vars
                  # passphrase and the install prompts for nothing. Token unlock needs only
                  # the post-boot systemd-cryptenroll; the crypttabExtraOpts that :348 stops
                  # emitting here is not required for it, see default.nix.
                  enrollFido2 = false;
