...
Record these verdicts as a Gate 1 modality verdict table in design.md carrying one row per requirement with an explicit `modality` column and an explicit `stratum` column; the `modality` column is the machine-checkable field that the pre-apply EST layout and the before-archive reconciliation both read to decide, per requirement, whether an EST witness is expected, and the `stratum` column records the requirement's capability tag from the proposal.
...
The stratum column extends the same table rather than adding a second one beside it, because the stratum is an input to the modality verdict, not a parallel axis: a capability's `world`, `interface`, or `behavioral` tag, set in the proposal per the schema's Capabilities instruction, constrains which modality Gate 1's proposition-class routing can actually reach for that requirement.
A `behavioral`-stratum requirement is the only stratum eligible for a Gherkin scenario in the usual sense: business-facing ubiquitous-language steps bound through `bdd-step-definitions`, following Gate 1's four behavioral rows unchanged.
A `world`-stratum requirement never routes to a Gherkin scenario, regardless of what its proposition class alone would suggest.
Its OpenSpec scenario is already a violation or monitoring condition in the schema's required form, `WHEN <the assumption drifts> THEN <the assumption is void and the requirements relying on it lose their discharge>`, and that condition is the witness: a `world` requirement states what is true regardless of what is built, not a system behavior a stakeholder observes at the surface, so it is recorded in design.md with `modality` set to this violation-condition witness rather than `BDD scenario`.
An `interface`-stratum requirement that Gate 1's proposition class would otherwise send to a BDD scenario is instead witnessed at the machine boundary: its scenario may mention only phenomena shared between world and machine, per the specification stratum's alphabet restriction, and a step definition binds directly to that interface-level observation rather than through the domain-language mediation `bdd-gherkin-formulation` provides for `behavioral` scenarios.
`preferences-requirements-engineering` owns the world/specification distinction the stratum tag operationalizes per capability; point there for the pentad, not here.
...
It also points to `preferences-requirements-engineering` for the world/specification distinction the verdict table's stratum column operationalizes.
