...
Symmetric to the `.feature` layout for behavioral requirements, a requirement Gate 1 routes to an executable-specification modality has its EST test laid out alongside the feature layout in the same pre-apply step, with a corresponding tasks.md entry the apply-change executor drives during red-green; the openspec-bdd-bridge skill owns this layout and keys it to a machine-checkable `modality` column in the change's design.md Gate 1 verdict table.
This EST arm inherits the same no-op-on-empty guarantee as the behavioral arm above: when no delta-spec requirement carries an EST-routed modality, nothing is laid out and the gate is byte-identical to today, so AFK and Manual spec-less paths remain unaffected, and it composes as an additive step that adds no board or Linear state, in the manner the Future automation extension point below sanctions.
