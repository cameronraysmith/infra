---
name: preferences-discovery-process
description: Discovery process methodology for exploring problem domains before implementation. Load when starting new projects or investigating unfamiliar domains.
---
...
### Step 7: Formalize specifications
...
Event Modeling is a self-contained methodology that does not require prior EventStorming.
Teams can start from natural language prompts describing workflows and generate complete specifications suitable for code generation and EventCatalog documentation.

If teams have previously run EventStorming sessions, those artifacts can optionally enrich Event Modeling's brainstorming phase.
This integration is optional enrichment, not a prerequisite.

Direct algebraic translation applies when teams prefer to move immediately from informal artifacts to type specifications without intermediate tooling.
Domain events become sum type constructors, commands become validated functions, aggregates become Decider modules, and policies become event handlers.

See `event-modeling.md` for the complete 7-step methodology and Qlerify tooling guidance.
See `domain-modeling.md` for direct translation patterns from discovery artifacts to algebraic types.
