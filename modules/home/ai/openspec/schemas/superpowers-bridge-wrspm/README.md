...
| Test backfill / coverage | (no) No | Direct PR |
| Build tooling tweak (linter rule, coverage threshold) | (no) No | Direct PR |
| Non-breaking dependency upgrade | (no) No | Direct PR |
| Documentation update / typo fix | (no) No | Direct PR |
| Config value tweak (no structural change) | (no) No | Direct PR |
...
> `artifactPaths.<id>.existingOutputPaths`, `planningHome.changesDir`, `changeRoot`), verified
> against 1.10.0 status JSON.
>
> Also at this baseline: the WRSPM stratum layer was added — a per-capability `world` /
> `interface` / `behavioral` tag in `proposal`, stratum-conditional vocabulary rules in `specs`,
> a non-blocking §8 in `verify` (designation lint, discharge coherence, alphabet check), and
> satisfaction-projection regeneration in the apply phase's archive step. The per-task
> `— verify:` convention and the closing `## Integration Verification` group were adopted from
> upstream 1.10.0's own tasks template, which this bundle's template had not tracked.
>
> Known upstream defect at this baseline: `openspec schema fork` fails against a read-only
> source tree (it preserves mode-444 on its staging copy, then cannot write it). Stage a
> project-tier schema with a whole-directory symlink instead.
